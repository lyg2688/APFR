...
class Engine(object):
    ...
    def fed_train_single_batch(self, model_client, batch_data, optimizers):
        ...
        optimizer.zero_grad()
        # ratings_pred = model_client(items, ratings.long())
    ...
    def fed_train_a_round(self, all_train_data, round_id):
        ...
        for user in participants:
            ...
            for key in self.client_model_params[user].keys():
                ...
                if key == "embedding_item.weight" and self.config['noise'] != 0:
                    if self.config['noise_function'] == "laplace_function":
                        self.client_model_params[user][key] = self.client_model_params[user][key] - laplace_function(self.config['noise'], self.client_model_params[user][key])
                    if self.config['noise_function'] == "gauss_function":
                        self.client_model_params[user][key] = self.client_model_params[user][key] - gauss_function(
                            self.config['noise'], self.client_model_params[user][key])
            ...
            for i in self.model.state_dict().keys():
                if 'affine_output' in i:
                    del round_participant_params[user][i]
            #
    ...
    def fed_evaluate(self, evaluate_data):
        ...
        for user in range(self.config['num_users']):
            ...
            with torch.no_grad():
                ...
                # test_score = user_model(test_item, torch.ones(len(test_item)).cuda())
                test_score = user_model(test_item)
                # negative_score = user_model(negative_item, torch.zeros(len(negative_item)).cuda())
...
def laplace_function(lambda_, x):
    return (1 / (2 * lambda_)) * np.e ** (-1 * (np.abs(x) / lambda_))


def gauss_function(lambda_, x):
    return (1 / np.sqrt(2 * lambda_) * lambda_) * np.e ** (-1 * (np.square(x) / (2 * np.square(lambda_))))
